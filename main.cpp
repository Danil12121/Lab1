...
int main(){
    Student anna;
    anna.initialize("Anna", "G1", {4, 5, 5, 3}, {"Geom", "Math", "Rus", "History"});
    Student ivan;
    ivan.initialize("Ivan", "G1", {4, 5, 5, 5}, {"Geom", "Math", "Rus", "History"});
    Student petr;
    petr.initialize("Petr", "G1", {2, 3, 4, 5}, {"Geom", "Math", "Rus", "History"});
    vector<Student> students = {ivan, anna, petr};
    ...
    g1.SortByRating();
    g1.show();
    cout << g1.CountTwoness() << endl;
    cout << g1.CountExcellent() << endl;

    vector<Student> exMath = g1.VectorMathExcellent();
    for (Student s : exMath)
        cout << s.GetName() << ' ';
    cout << endl;

    Student masha;
    masha.initialize("Masha", "G1", {2, 3, 2, 3}, {"Geom", "Math", "Rus", "History"});

    g1.AddStudent(masha);
    g1.show();

    Student x = g1.FindStudent("Masha");
    cout << x.GetName() << endl;
